...
from common import OUT, Usage, icon_b64, items, require_key, save_jsonl  # noqa: E402
...
def main() -> None:
    ...
    save_jsonl(OUT / ("labels_%s.jsonl" % tag), out)
    report = usage.report(args.model, batch=not args.sync)
    (OUT / ("labels_%s.cost.json" % tag)).write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print("저장: out/labels_%s.jsonl · 비용 %s" % (tag, report))
